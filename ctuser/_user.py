...
from cantools.web import log, respond, succeed, fail, cgi_get, read_file, redirect, fetch, send_mail, send_sms
from cantools.web.util import verify_recaptcha
...
def response():
    ...
    if action == "achek":
        ...
        if ecfg.scheduler:
            succeed(fetch("https://%s/_user?action=esched"%(ecfg.scheduler,), timeout=8, ctjson=True))
        else:
